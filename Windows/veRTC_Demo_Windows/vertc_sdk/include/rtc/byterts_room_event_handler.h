...
public:
     ...
     *        2. 新进房用户会收到进房前已在房内的可见用户的进房回调通知。  <br>
     ...
     * @param [in] uid 离开房间，或切至不可见的的远端用户 ID。  <br>
     * @param [in] reason 用户离开房间的原因：  <br>
     *              + 0: 远端用户调用 leaveRoom{@link #IRTSRoom#leaveRoom} 主动退出房间。  <br>
     *              + 1: 远端用户因 Token 过期或网络原因等掉线。 <br>
     *              + 2: 服务端调用 OpenAPI 将远端用户踢出房间。
     */
