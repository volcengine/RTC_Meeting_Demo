			...
			bytertc::LocalVideoStreamError error) {
				if (state == bytertc::kLocalVideoStreamStateFailed &&
					...
					if (!meeting::DataMgr::instance().mute_video()) {
						...
						if (error != bytertc::kLocalVideoStreamErrorDeviceNoPermission)
						{
...
void MeetingRtcEngineWrap::onAudioStateChanged(std::string device_id, 
	...
	if ((devices.empty() && !meeting::DataMgr::instance().mute_audio())) {
	...
	case bytertc::kMediaDeviceStateRemoved:  // Pull out the device
    {
        ...
        MeetingRtcEngineWrap::getAudioInputDevice(curDeviceStr);
    ...
    case bytertc::kMediaDeviceStateRuntimeError:
    {
        std::string curDeviceStr;
		MeetingRtcEngineWrap::getAudioInputDevice(curDeviceStr);
        if (strcmp(curDeviceStr.c_str(), device_id.c_str()) == 0 && !meeting::DataMgr::instance().mute_audio()) {
            if (error == bytertc::kMediaDeviceErrorDeviceNoPermission
                || error == bytertc::kMediaDeviceErrorDeviceFailure
                || error == bytertc::kMediaDeviceErrorDeviceBusy) {
                WarningTips::showTips("麦克风权限已关闭，请至设备设置页开启",
                    TipsType::kWarning,
                    meeting::PageManager::currentWidget(), 2000);
                meeting::DataMgr::instance().setMuteAudio(true);
				MeetingRtcEngineWrap::muteLocalAudio(true);
