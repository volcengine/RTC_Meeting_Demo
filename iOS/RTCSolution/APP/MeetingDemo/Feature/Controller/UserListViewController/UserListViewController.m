...
- (void)muteAllButtonAction {
    [MeetingRTSManager muteUser:@"" block:^(BOOL result, RTSACKModel * _Nonnull model) {
        if (!model.result) {
...
- (void)muteUserOrAskMic:(RoomVideoSession *)videoSession {
    ...
    if (isOpenAudio) {
        //mute microphone
        [MeetingRTSManager muteUser:videoSession.uid block:^(BOOL result, RTSACKModel * _Nonnull model) {
                    
        }];
    } else {
        //Ask microphone On
        [MeetingRTSManager askMicOn:videoSession.uid block:^(BOOL result) {
...
- (void)changeHost:(NSString *)uid {
    [MeetingRTSManager changeHost:uid block:^(BOOL result, RTSACKModel * _Nonnull model) {
        if (!model.result) {
            [[ToastComponent shareToastComponent] showWithMessage:model.message];
        }
    }];
...
- (BaseButton *)muteAllButton {
    if (!_muteAllButton) {
        ...
        [_muteAllButton bingImage:[UIImage imageNamed:@"meeting_par_muteall" bundleName:HomeBundleName] status:ButtonStatusNone];
