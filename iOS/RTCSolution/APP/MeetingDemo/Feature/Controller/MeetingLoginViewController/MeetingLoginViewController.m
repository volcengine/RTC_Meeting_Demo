...
- (void)viewDidLoad {
    ...
    [self loadDataWithGetAppIDWithUid];
...
- (void)applicationBecomeActive {
    ...
    if ([topVC isKindOfClass:[MeetingLoginViewController class]]) {
        ...
        alertModel.title = @"确定";
        [[AlertActionManager shareAlertActionManager] showWithMessage:@"本产品仅用于功能体验，单次会议时长不超过20分钟"
                                                              actions:@[alertModel]];
    }
}
...
- (void)onClickEnterRoom:(UIButton *)sender {
    ...
    if([[NetworkReachabilityManager sharedManager] isDisconnect]) {
        return;
    }
    [[ToastComponent shareToastComponent] showLoading];
    __weak __typeof(self) wself = self;
    [MeetingRTSManager joinMeeting:roomVideoSession block:^(NSString * _Nonnull token, NSArray<RoomVideoSession *> * _Nonnull userLists, RTSACKModel * _Nonnull model) {
        if (model.result) {
...
- (void)loadDataWithGetAppIDWithUid {
    __weak __typeof(self) wself = self;
    [MeetingRTSManager getAppIDWithUid:@"" roomId:@"" block:^(NSString * _Nonnull appID, RTSACKModel * _Nonnull model) {
        if (model.result) {
...
- (BaseButton *)enableAudioBtn {
    if (!_enableAudioBtn) {
        ...
        [_enableAudioBtn bingImage:[UIImage imageNamed:@"meeting_room_mic" bundleName:HomeBundleName] status:ButtonStatusNone];
        [_enableAudioBtn bingImage:[UIImage imageNamed:@"meeting_room_mic_s" bundleName:HomeBundleName] status:ButtonStatusActive];
        [_enableAudioBtn bingImage:[UIImage imageNamed:@"meeting_room_mic_s" bundleName:HomeBundleName] status:ButtonStatusIllegal];
...
- (BaseButton *)enableVideoBtn {
    if (!_enableVideoBtn) {
        ...
        [_enableVideoBtn bingImage:[UIImage imageNamed:@"meeting_login_video" bundleName:HomeBundleName] status:ButtonStatusNone];
        [_enableVideoBtn bingImage:[UIImage imageNamed:@"meeting_room_video_s" bundleName:HomeBundleName] status:ButtonStatusActive];
        [_enableVideoBtn bingImage:[UIImage imageNamed:@"meeting_room_video_s" bundleName:HomeBundleName] status:ButtonStatusIllegal];
...
- (UIButton *)setingBtn {
    if (!_setingBtn) {
        ...
        [_setingBtn setImage:[UIImage imageNamed:@"meeting_login_set" bundleName:HomeBundleName] forState:UIControlStateNormal];
...
- (UIImageView *)logoImageView {
    if (!_logoImageView) {
        ...
        _logoImageView.image = [UIImage imageNamed:@"logo_icon" bundleName:HomeBundleName];
...
- (BaseButton *)navLeftButton {
    if (!_navLeftButton) {
        ...
        [_navLeftButton setImage:[UIImage imageNamed:@"meeting_nav_left" bundleName:HomeBundleName] forState:UIControlStateNormal];
