    ...
    if (status == RoomBottomStatusMic) {
        ...
        if (itemButton.status == ButtonStatusNone) {
            if ([self.currentRoomModel.host_id isEqualToString:self.localVideoSession.uid]) {
                ...
                __weak typeof(self) weakSelf = self;
                [MeetingRTMManager recordMeeting:[self getRecordUids]
...
- (void)loadDataWithMeetingContrller:(BOOL)isUserInfo {
    __weak __typeof(self) wself = self;
    [MeetingRTMManager getMeetingInfoWithBlock:^(MeetingControlRoomModel * _Nonnull roomModel, RTMACKModel * _Nonnull model) {
        if (model.result) {
...
- (void)meetingReconnect {
    __weak __typeof(self) wself = self;
    [MeetingRTMManager reconnectWithBlock:^(RTMACKModel * _Nonnull model) {
        NSString *type = @"";
        if (model.result) {
            type = @"resume";
...
- (void)getMeetingUserInfo {
    __weak __typeof(self) wself = self;
    [MeetingRTMManager getMeetingUserInfo:@"" block:^(NSArray<RoomVideoSession *> * _Nonnull userLists, RTMACKModel * _Nonnull model) {
        //Refresh now render view
        ...
        if (isAuthorize) {
            if (self.screenComponent.isSharing) {
                ...
                if (itemButton.status == ButtonStatusActive) {
                    ...
                    [MeetingRTMManager turnOnCamera];
                }
            }
        } else {
...
- (void)updateRecordLayout:(NSArray<NSString *> *)uids screenId:(NSString *)screenId {
    ...
    if (![temoRecordID isEqualToString:self.recordID] && self.currentRoomModel.record) {
        ...
        [MeetingRTMManager updateRecordLayout:uids screenId:screenId];
    }
}
